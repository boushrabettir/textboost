...
import os
#TODO pip install/install imports below
import pdf2htmlEX
import markdownify
import pandoc
...
def html_to_md(path: str) -> str:
    ...
    try:
        ...
        print(f"File '{temp_txt_path}' does not exist.")
...
def md_to_pdf(name: str, folder: str) -> None:
    ...
    try:
        subprocess.run(
            ["mdpdf","-o",f"modified/{folder}/{name}.pdf",f"modified/{folder}/{name}.md"]
        )
    except subprocess.CalledProcessError:
